...
public:
	...
	bool isFullScreen = false;	// 默认全屏
	int ScreenID = 0;	// 若有多块屏幕，全屏时在哪一块屏幕上 【暂未使用】

	unsigned int scr_windth = SCR_WIDTH;
	unsigned int scr_height = SCR_HEIGHT;

	XYY_Window(unsigned int scr_windth = SCR_WIDTH, unsigned int scr_height = SCR_HEIGHT,bool isFull = true, unsigned int gl_version_major = 3, unsigned int gl_version_minor = 3);
