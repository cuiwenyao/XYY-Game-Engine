...
XYY_MeshResource XYY_ModelResource::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	{
		Vertex vertex;
		glm::vec3 vector; // we declare a placeholder vector since assimp uses its own vector class __declspec(dllexport) that doesn't directly convert to glm's vec3 class __declspec(dllexport) so we transfer the data to this placeholder glm::vec3 first.
		// positions
